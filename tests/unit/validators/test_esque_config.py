...
def sample_config_data() -> typing.Dict:
    return yaml.safe_load(get_path_for_config_version(LOAD_SAMPLE_CONFIG))


def test_sample_config_valid(interactive_cli_runner: click.testing.CliRunner, sample_config_data: typing.Dict):
    validate_esque_config(sample_config_data)


def test_mixed_case_security_protocol(sample_config_data: typing.Dict):
    sample_config_data[sample_config_data["current_context"]]["security_protocol"] = "sasL_PLaintEXT"
    validate_esque_config(sample_config_data)


def test_invalid_security_protocol(sample_config_data: typing.Dict):
    sample_config_data[sample_config_data["current_context"]]["security_protocol"] = "sasLaintEXT"
    with pytest.raises(esque.errors.ValidationException):
        validate_esque_config(sample_config_data)


def test_mixed_case_sasl_mechanism(sample_config_data: typing.Dict):
    sample_config_data[sample_config_data["current_context"]]["sasl_params"]["mechanism"] = "SCRAM-sha-512"
    validate_esque_config(sample_config_data)


def test_invalid_sasl_mechanism(sample_config_data: typing.Dict):
    sample_config_data[sample_config_data["current_context"]]["sasl_params"]["mechanism"] = "SCRAM-sh-512"
    with pytest.raises(esque.errors.ValidationException):
        validate_esque_config(sample_config_data)
